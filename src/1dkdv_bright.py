...
u0 = 1/torch.cosh(x + 2) # initial guess bright soliton
...
for epoch in range(number_of_epoch):
  ...
  # Step the scheduler
  #scheduler.step()
  loss_value_kdv.append(total_loss.item())
...
def closure():
    ...
    lbfgs_loss_value_kdv.append(total_loss.cpu().item())
    return total_loss
...
upred1kdv = Net.forward(X_tensor).detach().cpu().numpy()
plt.plot(X_tensor, abs(upred1kdv))
plt.xlabel("x")
plt.ylabel("u")
plt.title("pred")
plt.show()
...
utrue_kdv = (1/2)*(1/np.cosh(0.5*(xx + 1)))**2 # bright
uguess = 1/np.cosh(xx+2)
plt.plot(xx, utrue_bright, label="true")
...
plt.plot(X_tensor, abs(upred1kdv), label="pred")
plt.plot(xx, abs(utrue_kdv), label="true")
plt.xlabel("x")
plt.ylabel("u")
plt.legend()
plt.title("compare")
