...
# 1 D NLSE with Harmonic gaussian Potential
# L u = 0
# u_xx + |u|^2u + mu*u - (x^2 - V0e^(-x^2))u = 0
#mu =1, V0 = -4 , g = -1
# u(x) = sqrt(V0/g) e^((-x^2)/2)
# u_0 = e^((-x^2)/2)
# domain [-20,20] , N =500
...
u0 = 4*x*torch.exp((-x**2)/2) # initial guess dipole mode
...
def ode_residual(Net, x):
    ...
    ode = u_xx  +  u**3 + mu*u - (x1**2 - V0*torch.exp(-x**2))*u
...
optimizer_lbfgs = torch.optim.LBFGS(Net.parameters(), max_iter=1000, line_search_fn="strong_wolfe")
...
utrue = np.sqrt(V0/g)*np.exp((-xx**2)/2) # dark
uguess = np.exp((-xx**2)/2)
